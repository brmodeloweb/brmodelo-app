...
describe('validateSaveModelParams test', () => {
  ...
  test('should fail when name has a wrong type', () => {
    ...
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().WRONG_NAME_TYPE);
  });
  ...
  test('should fail when type is empty', () => {
    const validation = modelValidator.validateSaveParams({name: "Teste", type: ""});  
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().MISSING_TYPE);
  });

  test('should fail when has a wrong type', () => {
    const validation = modelValidator.validateSaveParams({name: "Teste", type: 5});  
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().WRONG_TYPE_TYPE);
  });

  test('should fail when model is missing', () => {
    const validation = modelValidator.validateSaveParams({name: "Teste", type: "conceltual"});  
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().MISSING_MODEL);
  });

  test('should fail when model is empty', () => {
    const validation = modelValidator.validateSaveParams({name: "Teste", type: "conceltual", model: {}});  
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().MISSING_MODEL);
  });

  test('should fail when model a wrong type', () => {
    const validation = modelValidator.validateSaveParams({name: "Teste", type: "conceptual", model: "teste"});  
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().WRONG_MODEL_TYPE);
  });

  test('should fail when user is missing', () => {
    const validation = modelValidator.validateSaveParams({name: "Teste", type: "conceltual", model: {id: "1234"}});  
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().MISSING_USER);
  });

  test('should fail when user is 0', () => {
    const validation = modelValidator.validateSaveParams({name: "Teste", type: "conceltual",  model: {id: "1234"}, userId: ""});  
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().MISSING_USER);
  });

  test('should fail when model a wrong type', () => {
    const validation = modelValidator.validateSaveParams({name: "Teste", type: "conceptual",  model: {id: "1234"}, userId: 5});  
    expect(validation.valid).toBeFalsy();
    expect(validation.message).toBe(modelValidator.getMessages().WRONG_USER_TYPE);
  });
});