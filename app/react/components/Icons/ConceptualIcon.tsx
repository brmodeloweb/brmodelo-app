import React from "react";
import styled from "styled-components";
import { useTranslation } from "react-i18next";

const IconWrapper = styled.div`
	position: relative;
	display: inline-block;

	svg {
		display: block;
	}
`;

type ConceptualIconProps = {
	height?: string;
	className?: string;
};

const ConceptualIcon: React.FC<ConceptualIconProps> = React.memo(({
	height = "20",
	className
}) => {
	const { t } = useTranslation(["common"]);

	return (
		<IconWrapper className={className} title={t("Conceptual model")}>
			<svg xmlns="http://www.w3.org/2000/svg" height={height} viewBox="0 -960 960 960">
				<path d="M479.995-95.463q-59.995 0-102.033-42.042-42.038-42.042-42.038-102.034 0-53.265 33.615-93.171 33.616-39.906 84.462-48.888v-84.403h-206v-111.922H141.925v-264.152h264.152v264.152H300v59.924h372v-62.386q-50.846-8.538-84.461-48.444-33.616-39.906-33.616-93.17 0-59.992 42.044-102.034 42.043-42.042 102.038-42.042 59.994 0 102.032 42.042t42.038 102.034q0 53.264-33.615 93.17t-84.461 48.444v114.384h-218v84.403q50.846 8.982 84.462 48.888 33.615 39.906 33.615 93.171 0 59.992-42.044 102.034-42.043 42.042-102.037 42.042Zm218.227-534.459q38.239 0 65.047-27.031t26.808-65.269q0-38.239-27.031-65.047t-65.27-26.808q-38.239 0-65.046 27.031-26.808 27.031-26.808 65.27t27.031 65.046q27.031 26.808 65.269 26.808Zm-504.299 0h160.155v-160.155H193.923v160.155Zm286.3 482.46q38.239 0 65.047-27.031t26.808-65.269q0-38.239-27.031-65.047t-65.27-26.808q-38.239 0-65.047 27.031t-26.808 65.27q0 38.238 27.031 65.046 27.031 26.808 65.27 26.808ZM274.001-709.999Zm423.998-12ZM480-239.539Z" />
			</svg>
		</IconWrapper>
	);
});

ConceptualIcon.displayName = 'ConceptualIcon';
export default ConceptualIcon;