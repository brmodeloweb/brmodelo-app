...
import { INoSqlEditor, CollectionOption, NoSqlUpdateEvent } from "./INoSqlEditor";
import NoSqlEditor from "./nosqlEditor";
import PanelTools from "../../components/PanelTools";
import { NoSqlIcon } from "../../components/Icons";
import Feedback, { useFeedback } from "../../components/Feedback";
import NoSqlSidebar from "./sidebar/NoSqlSidebar";
...
const NoSqlPage: React.FC<NoSqlPageProps> = React.memo(({
	modelId,
	conversionId,
	...
	const [model, setModel] = useState<Model>({ id: "", name: "", type: "nosql", updatedAt: "" });
	...
	const refreshCollections = useCallback(() => {
		const editor = editorRef.current;
		if (!editor) return;
		const list = editor.loadCollections().map((c: any) => ({ name: c.name, id: c.id }));
		setCollections(list);
	}, []);
	...
	const handleUpdate = useCallback((event: NoSqlUpdateEvent) => {
		const editor = editorRef.current;
		if (!editor) return;
		...
		if (event.element && event.element.model?.attributes?.type !== undefined) {
			const enriched: SelectedElement = {
				value: event.element.model?.getName?.() || "",
	...
	useEffect(() => {
		...
		const customActions = {
			...
			selectRow: (rowPath: number[]) => {
				setSidebarVisible(true);
				setFocusedRowPath(rowPath);
			},
			saveModel: () => {
		...
		const editor = new NoSqlEditor(
			document,
			contentRef.current,
			conversionOptionModalService,
			customActions,
		) as unknown as INoSqlEditor;
		...
		const loadModel = async () => {
			...
			try {
				...
				if (conversionId) {
					...
					await (editor as any).toNoSql(conceptualJson);
					refreshCollectionsRef.current();
	...
	}, [modelId, conversionId, conversionOptionModalService, onLoadModel, onNavigateToWorkspace, showFeedback, hideFeedback, onEditorReady, onConversionConsumed]);
			...
			<section className="modelWorkspace">
				<header className="modelWorkspace-header">
					<button
						...
						aria-label={t("Back to Model list")}
				...
				<section id="content" className="pseudo-canvas" ref={contentRef}>
					...
					<NoSqlSidebar
						...
						onFocusedRowConsumed={() => setFocusedRowPath(null)}
...
NoSqlPage.displayName = "NoSqlPage";
